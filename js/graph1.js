...
function legacy_dir_to_vector(dir_) {
  let threevector = new THREE.Vector3(1,0,0);
  if (dir_ == 'E') {
    // threevector.applyAxisAngle( new THREE.Vector3(0,0,1), Math.PI/3 );
  } else if (dir_ == 'SE') {
    threevector.applyAxisAngle( new THREE.Vector3(0,0,1), Math.PI/3 );
  } else if (dir_ == 'SW') {
    threevector.applyAxisAngle( new THREE.Vector3(0,0,1), Math.PI*2/3 );
  } else if (dir_ == 'W') {
    threevector.applyAxisAngle( new THREE.Vector3(0,0,1), Math.PI );
  } else if (dir_ == 'NW') {
    threevector.applyAxisAngle( new THREE.Vector3(0,0,1), Math.PI*4/3 );
  } else if (dir_ == 'NE') {
    threevector.applyAxisAngle( new THREE.Vector3(0,0,1), Math.PI*5/3 );
  } else if (dir_ == 'E') {
  } else {
    console.log('no legacy direction available');
  }
  console.log(threevector);
  return(threevector);
}
  ...
  if (arrayoflines[0] == 'legacy') {
    for (line of arrayoflines) {
      ...
      if ((rule != undefined) && (rule != '')) {
        ...
        if (phase == 'bonds') {
          let x1 = parseFloat(rule[0]);
          let y1 = parseFloat(rule[1]);
          let z1 = 0;
          if (rule.length == 7) {
            z1 = parseFloat(rule[2]);
          }
          let x2 = parseFloat(rule[2]);
          let y2 = parseFloat(rule[3]);
          let z2 = 0;
          if (rule.length == 7) {
          ...
          for (node of nodes) {
            ...
            if (node1 != undefined && node2 != undefined) {
              if (rule[rule.length-1] = '1') {
                edges.push(new Edge(node1, node2, type_ = 'rigid'))
              } else if (rule[rule.length-1] = '2') {
                edges.push(new Edge(node1, node2, type_ = 'flexible'))
              }
            } else {
              console.log('nodes do not exist for bond');
            }
          }
        }
        ...
        if (phase = 'rules') {
          ...
          let dir_i = legacy_dir_to_vector(rule[3]);
          let dir_f = legacy_dir_to_vector(rule[7]);
            ...
            if (boundnode.state == rules[n]['state2_i']) { // check that neighbour in that direction is correct state
              if (boundedge.type == rules[n]['type_i']) { // check that edge is correct type
                ...
                if (rules[n].dir_i != rules[n].dir_f) { // if the direction must change
                  ...
                  if (boundnode.move_all(relativedirection)) {
                    rule_applied = true;
                  } else {
